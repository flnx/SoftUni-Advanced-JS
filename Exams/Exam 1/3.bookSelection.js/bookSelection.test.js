...
describe('Book Selection', () => {
  ...
  describe('isItAffordable Test', () => {
    ...
    it('should throw error when params are not numbers', () => {
      expect(() => bookSelection.isItAffordable(3, [])).to.throw(`Invalid input`);
  ...
  describe('suitableTitles test', () => {
    it('throws Invalid input when the parameters are invalid', () => {
      expect(() => bookSelection.suitableTitles(['test', 'testov'], 5)).to.throw(
        'Invalid input'
      );
      expect(() => bookSelection.suitableTitles('lili', 'ivanova')).to.throw(
        'Invalid input'
      );
      expect(() => bookSelection.suitableTitles(5, ['test', 'testov'])).to.throw(
        'Invalid input'
      );
      expect(() => bookSelection.suitableTitles(['test', 'testov'], 5)).to.throw(
        'Invalid input'
    ...
    it('returns the result when parameters are valid', () => {
      ...
      expect(bookSelection.suitableTitles(test, 'Crime')).to.deep.equal([`The Da Vinci Code`]);
